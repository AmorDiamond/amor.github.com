  ...
  $.fn.autoMove = function(args){
    ...
    return this.each(function(){
      ...
      that.on('mousedown', function(e){
        ...
        var oldX = e.pageX,oldY = e.pageY;
        var emX = $(that).offset().left,emY = $(that).offset().top;
        that.on('mousemove', function(e){
          var x = e.pageX,y = e.pageY;
          var moveX = x - oldX,moveY = y - oldY;
          var emW = $(that).width(),emH = $(that).height();
          var positionX = Math.floor(emX + moveX),positionY = Math.floor(emY + moveY);
          // that.css({'left': x - emW/2,'top': y - emH/2});
          that.css({'left': positionX,'top': positionY});
        });
      });//添加鼠标事件
